...
def simplify(url: str, keys: list) -> Result:
    try:
        ...
        if original_domain not in allowed_domains:
            return Result(code="-1", message=f"URL域名不支持，仅支持以下域名：{', '.join(allowed_domains)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--url", required=True, type=str, help="抖店商品链接")
    parser.add_argument("--output", required=True, type=str, help="输出结果所在的json文件路径")
    args = parser.parse_args()
    
    simplify_res = simplify(args.url, ["id"])
    if simplify_res.code != "0": perror(simplify_res)
    print(f"简化URL成功，简化后的URL: {simplify_res.message}")

    submit_res = submit(3296206833079096, json.dumps({"url": simplify_res.message}, ensure_ascii=False))
