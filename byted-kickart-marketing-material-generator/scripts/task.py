...
def submit(service_id: int, params: str)->Result:
    try:
        ...
        response = do_request("POST", {}, submit_bytes, action="SubmitAiTemplateTaskAsync").json()
        ...
        if not task_id or not task_id[0]: return Result(code="-1", message=f"解析TaskId失败, 响应内容: {response.c}")
...
# 轮询任务状态
def poll(task_id: str, max_attempts: int = 10 * 10, interval_seconds: int = 6)->Result:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="根据任务ID重新查询任务结果")
    parser.add_argument("--id", required=True, type=str, help="任务ID")
    parser.add_argument("--output", required=True, type=str, help="输出结果所在的json文件路径")
    args = parser.parse_args()
    
    poll_res = query(args.id)
