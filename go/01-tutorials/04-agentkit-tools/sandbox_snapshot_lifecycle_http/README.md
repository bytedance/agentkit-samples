...
- `AGENTKIT_LIFECYCLE_STATE`：七个命令共享的状态文件路径；默认使用本目录的
...
`https://agentkit.<region>.volces.com`。命令 02 会自动选择该地址；如通过
...
七个命令通过 `.sandbox_snapshot_state.json` 传递 `tool_id`、逻辑会话 ID、沙箱实例
