# 沙箱 Session 与快照生命周期 HTTP 示例（Go）

这个目录提供一套不依赖 AgentKit SDK 的 Go 示例。代码直接调用 AgentKit Tools
OpenAPI，通过 `internal/lifecycle` 中的标准库 HTTP client 完成请求、HMAC-SHA256
签名、错误解析、基础重试、状态文件读写和轮询等待。

这套示例与 Python 目录
`python/01-tutorials/04-agentkit-tools/sandbox_snapshot_lifecycle_http` 并行存在，
接口顺序和状态文件语义保持一致。带签名的 endpoint 中如果包含 `Authorization`
查询参数，示例会在输出或保存状态前自动脱敏。
...
- `AGENTKIT_SESSION_TTL_SECONDS`：Session 和恢复后实例的生命周期，默认 `28800`
  秒（8 小时）；命令 02 调用时不传入 TTL。
- `AGENTKIT_USER_SESSION_ID`：逻辑会话 ID；如果不指定，命令 01 会自动生成。
- `AGENTKIT_INVOKE_CODE`：命令 02 执行的 Python 代码，默认
  `print('Hello from AgentKit sandbox!')`。
- `AGENTKIT_INVOKE_TIMEOUT_SECONDS`：命令 02 的代码执行超时，默认 30 秒，必须为正整数。
- `AGENTKIT_INVOKE_KERNEL_NAME`：命令 02 使用的内核，默认 `python3`。
- `AGENTKIT_LIFECYCLE_STATE`：八个命令共享的状态文件路径；默认使用本目录的
...
`InvokeTool`、`AsyncExecCommand` 和 `ViewAsyncCommand` 使用数据面地址：
火山引擎为 `https://agentkit.<region>.volces.com`，BytePlus 为
`https://agentkit.<region>.bytepluses.com`；BytePlus 管理接口仍使用 `byteplusapi.com`。
同步和异步调用均自动选择地址；若设置当前云平台的 host 覆盖，
该地址必须支持所调用的数据面 Action。

## 调用 Session 执行代码

`02_invoke_session` 调用 `InvokeTool`，在状态文件记录的沙箱实例中执行 Python 代码。
请在命令 01 创建实例并等待就绪后调用，或在命令 05 从快照恢复并等待就绪后再次调用。
...
其中包含 `code`、`timeout` 和 `kernel_name`。本示例显式调用已有 `SessionId`，
...
`02_async_invoke_session` 先调用 [AsyncExecCommand](https://docs.volcengine.com/docs/agentkit/AsyncExecCommand_-_Asynchronously_executes_a_Shell_command_in_a_tool?lang=zh)
...
沙箱镜像需要支持这两个 API 对应的异步 Shell 执行能力。复用 `internal/lifecycle` 的标准库 HTTP 客户端、签名和数据面地址选择，无需 AgentKit SDK。
...
在本示例目录执行，可替换或补充主流程中每次 `02_invoke_session` 调用：
...
默认 TTL 为 8 小时。如需缩短演示等待时间，可在运行命令 01 **之前**设置较短的
TTL，例如：
...
在本目录执行步骤 01-04，创建并调用 Session，然后创建和查询快照：
...
1. `01_create_session`：检查 Tool 是否开启快照，创建 Session，并等待沙箱实例进入
   `Ready`。
2. `02_invoke_session`：通过 `InvokeTool` 在已有 Session 中执行 Python 代码，
   校验实例 ID 并保存执行结果；恢复后可再次运行。
   步骤 02 也可使用 `02_async_invoke_session` 异步执行 Shell 命令并轮询；恢复后可再次运行。
3. `03_create_snapshot`：主动为 Session 对应的沙箱实例创建快照，并等待快照进入
   `Ready`。
4. `04_list_and_get_snapshot`：分页列出 Tool 下的全部快照，并单独获取刚创建的
   快照详情。
5. `05_restore_from_snapshot`：等待原 Session 生命周期结束后，以
   `CreateNewInstance=false` 从快照恢复原沙箱，并校验恢复后的 `SessionId` 必须与
   命令 01 保存的实例 ID 一致。如果实例仍在使用或处于异步终止状态，命令会自动等待并
   重试。
6. `06_delete_snapshot`：删除状态文件记录的快照，分页检查 Tool 下的快照列表，并
   等待目标快照彻底消失。该命令不会删除命令 05 恢复出来的沙箱实例。
7. `07_delete_session`：最后删除 Session。主动调用 `DeleteSession` 也会删除该
   Session 对应的其他剩余快照，因此不能用它作为命令 05 恢复前结束生命周期的手段。

等待原 Session 的 TTL 到期，生命周期结束后，再运行步骤 05。命令 01 输出的
...
或 `InvalidSnapshot.InstanceTerminating`（实例正在终止）时，命令会按轮询间隔重试，
直到允许恢复或达到等待超时。超时后可等 Session 生命周期结束再重跑步骤 05，
或增加 `AGENTKIT_WAIT_TIMEOUT_SECONDS`。其他 API 错误会直接返回。

步骤 05 成功后，可再次运行命令 02，确认恢复后的实例可以执行代码：
...
八个命令通过 `.sandbox_snapshot_state.json` 传递 `tool_id`、逻辑会话 ID、沙箱实例
ID 和快照 ID。命令 02 还保存 `invoked_at`、`invoke_response` 和 `invoke_result`，
