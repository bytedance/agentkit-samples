...
func run() error {
	...
	tool, err := client.GetTool(map[string]any{"ToolId": toolID})
	if err != nil {
		return err
	}
	if tool["EnableSnapshot"] != true {
		return fmt.Errorf("tool %s does not have EnableSnapshot=true; snapshots cannot be created for this tool", toolID)
	}
