...
本目录包含五个脚本，覆盖 Session 的创建、查询、暂停、恢复和删除。
...
根据沙箱所在云平台，选择下面一组配置。五个脚本共用同一个客户端配置。
...
| `03_pause_session.py` | 暂停状态文件中的 Session，等待 `Paused` 并记录 `paused_at`。 |
| `04_resume_session.py` | 要求状态文件中有 `paused_at`，恢复同一个 Session，校验实例 ID 不变并等待就绪。 |
| `05_delete_session.py` | 根据 Tool ID 和状态中的 `instance_id` 调用 `DeleteSession` 并保存响应，不等待后台删除完成。 |

验证暂停和恢复时，按 **01 → 02 → 03 → 04 → 02** 执行：先创建和查询，再暂停、
恢复，最后查询恢复结果。脚本 05 是删除操作，放在完成验证后清理资源时使用。
...
`user_session_id`、`instance_id`、生命周期时间和 API 响应。脚本 01、03、04、05
会写入状态，脚本 02 只读取。该文件名已被仓库根目录的 `.gitignore` 忽略；
通过 `AGENTKIT_LIFECYCLE_STATE` 使用其他文件名时，是否忽略取决于对应路径的 Git 规则。

脚本 03、04、05 根据状态文件中的 `instance_id` 操作实例，不读取
