#!/usr/bin/env python3
"""Create an eight-hour sandbox session and record its instance ID via HTTP."""
...
def main() -> None:
    ...
    client = new_client()
    tool = client.get_tool({"ToolId": tool_id})
    if tool.get("EnableSnapshot") is not True:
        raise RuntimeError(
    ...
    instance_id = str(response.get("SessionId") or "").strip()
    if not instance_id:
        raise RuntimeError("CreateSession response is missing SessionId")

    state = {
        "tool_id": tool_id,
        "user_session_id": response.get("UserSessionId") or user_session_id,
