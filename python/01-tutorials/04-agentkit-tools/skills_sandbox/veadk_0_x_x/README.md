...
| 使用内置 skills | 产物不上传 tos |  帮我分析一下 ByteHouse 集群 bh_log_boe 最近 1 小时的负载情况，输出 QPS 趋势、Top 表访问热度和当前正在执行的查询，并给出性能优化建议。 |
...
| 使用内置 skills | 产物上传 tos |  请运行以下工作流程：1. 使用 music-generate skill 帮我生成一段 60 秒的咖啡厅背景音乐 BGM，风格轻松慵懒，以钢琴和原声吉他为主，适合下午阅读时听，最后以拓展名标记产物音频格式。2. 使用 tos-file-access skill 把产物上传到存储桶 {YOUR_TOS_BUCKET_NAME} 里。 |
