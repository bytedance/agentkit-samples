...
| Use Built-in Skills | Product not uploaded to TOS | Please help me analyze the load status of the ByteHouse cluster bh_log_boe over the past 1 hour. Output the QPS trend, top table access heat, and currently executing queries, and provide performance optimization suggestions. |
...
| Use Built-in Skills | Product not uploaded to TOS | Please run the following workflow: 1. Use the music-generate skill to help me generate a 60-second cafe background music (BGM), with a relaxed and laid-back style, primarily featuring piano and acoustic guitar, suitable for listening during afternoon reading. Mark the artifact's audio format with its file extension at the end. 2. Use the tos-file-access skill to upload the artifact to the storage bucket {YOUR_TOS_BUCKET_NAME}. |
