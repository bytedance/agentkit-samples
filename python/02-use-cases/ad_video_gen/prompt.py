...
2. 一个图生视频：使用这张四宫格图片作为参考图，生成一条完整的商品营销视频。默认使用 Seedance 2.5 能力，生成 1080P、15 秒视频。
...
4. 拿到四宫格图片 URL 后，先立即用 Markdown 图片语法展示这张图片，并提醒用户：
   - “参考图已生成，接下来将使用 Seedance 2.5 生成视频。视频生成过程可能比较慢，通常需要几分钟，繁忙时可能达到十几分钟，请耐心等待。”
...
- 默认按 Seedance 2.5 支持的高质量规格生成：`resolution=1080p`、`duration=15`、`watermark=true`。
