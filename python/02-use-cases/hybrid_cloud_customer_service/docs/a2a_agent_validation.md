...
4. 注册完成后确认平台生成的 Card 预览中展示 Agent 名称、所需 Skill ID、版本和服务地址。

注册是平台治理和发现入口；一个 A2A 中心/空间可包含多个 Agent，平台会根据人工选中的
AgentCard 和 Runtime 服务地址定位对端。本 Demo 默认 Agent/Skill 为
`hybrid-cloud-complaint-data-agent` / `complaint-trend-analysis`，它们是业务验收默认值而非
A2A 中心的固定全局配置。不要把 Skills Space ID (`ss-...`) 或 Sandbox Tool ID (`t-...`) 填到 A2A 配置中。
...
2. 要求输入主 Runtime Region，以及从 A2A 中心详情复制的 Agent 名称、Skill ID 和服务地址；
   服务地址末尾未带 `/a2a` 时自动补齐；
3. 隐藏输入数据 Runtime 自己的 API Key；
...
1. 数据 Runtime 的 AgentCard 名称与 Skill ID 等于本次从 A2A 中心选择的值；
