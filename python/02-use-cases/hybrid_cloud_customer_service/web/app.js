...
const MODULES = {
  ...
  tools: {
    ...
    steps: [
      ...
      },
      {
        ...
        code: '请明确调用 execute_skills，按已发布的 customer-service-compliance Skill\n检查理财产品退款申请是否需要人工确认；返回 Skill 名称、合规结论和执行摘要。',
  ...
  a2a: {
    ...
    description: '本示例使用两个独立 Runtime：A2A 中心登记数据 Agent 的 Agent Card；主客服 Agent 读取 Card、校验 Skill，再通过标准 JSON-RPC message/send 调用对端 /a2a。当前调用地址由主 Runtime 显式配置，不会把本地函数伪装成跨 Agent 调用。',
    ...
    steps: [
      ...
      },
      {
        ...
        body: '发布数据 Agent 后，在 A2A 中心选择“注册 A2A 智能体 → 智能体运行时”，选中刚发布的 Runtime 与控制台自动带出的服务地址。平台读取并登记 Card 的名称、版本、能力和 Skill 清单；本示例声明 complaint-trend-analysis。',
        code: '注册来源：智能体运行时\n智能体运行时：<选择数据分析 Runtime>\n服务地址：<控制台自动带出>\n\n预期 Card Skill：complaint-trend-analysis',
        file: 'a2a_data_agent.py',
        note: '注册的是 Agent Card，不是主客服 Runtime，也不是 Skills Space。',
      },
      {
        title: '向客服主 Runtime 注入对端配置',
        body: '主 Agent 的直接调用客户端需要对端 RPC 地址和数据 Agent Runtime 自己的 API Key。只配置这两个变量即可；Card 地址默认由 /a2a 自动推导。Key 仅保存在主 Runtime 环境变量中，本地 UI 的 Runtime Key 只用于访问主 Runtime。',
      ...
      },
      {
        ...
        body: 'Agent Tool 运行在异步 ADK 调用中，因此使用 httpx.AsyncClient，避免同步网络请求阻塞事件循环。客户端先 GET Card 并确认 complaint-trend-analysis，再 POST /a2a；无论网络错误还是协议错误，工具都会返回明确结果，避免 ADK 出现 Missing tool results。',
    ...
    steps: [
      {
        ...
        body: '在 Skills 中心创建 Space，将 customer-service-compliance 等包加入该 Space。Space 是逻辑目录和元数据集合，不是 Sandbox Tool，也不是物理 Bucket。',
      ...
      },
      {
        ...
        body: '本环境已成功加载 customer-service-compliance。Runtime 日志中的 ListSkillsBySpaceId 和 Successfully loaded skill 是 Space/包治理侧的证据；实际执行证据请到“工具”页查看。',
        code: 'INFO ... ListSkillsBySpaceId request body: {"SkillSpaceId":"ss-..."}\nINFO ... Successfully loaded skill customer-service-compliance',
  ...
  identity: {
    title: 'Identity / 身份权限',
    description: 'Identity 是请求认证与业务身份边界；它不保存对话内容，也不等同于 Session。',
    prompt: '你是谁？请用一句话说明你的角色和可以帮助我的事项。',
    steps: [
      {
        title: '网关验证调用方',
        body: '调用方携带 OAuth JWT 或 Runtime API Key 访问 /invoke。AgentKit 网关先完成凭据校验；应用不在前端保存 Key，也不把用户身份当作普通 prompt 内容相信。',
        code: 'POST <runtime-endpoint>/invoke\nAuthorization: Bearer <oauth-jwt-or-runtime-api-key>\nuser_id: <caller-user>\nsession_id: <conversation-id>',
        note: '401 表示网关凭据缺失或无效；它与 Session 数据是否存在是两类问题。',
      },
      {
        title: '正确阅读在线测试响应',
        body: 'POST /invoke 返回 SSE 事件流。平台在线测试会展示完整原文；带 thought: true 或 partial: true 的 text 是模型思考片段，不是最终回答。HTTP 200 只证明鉴权、Origin 和 Runtime 路由已通过。',
        code: '思考事件：content.parts[].thought = true\n最终回答：content.parts[].text 且 thought != true\n本地 UI：自动分离思考事件，只在对话气泡展示最终回答',
        note: '在线测试需查看响应末尾的最终文本；也可以点击上方“身份自检”，由本地 UI 自动解析。',
      },
      {
        title: '验证身份边界',
        body: '先用有效 OAuth JWT 或 Runtime API Key 得到 200 和最终回答；移除或替换凭据应得到 401/403。Demo 中的 resolve_identity 还展示了 JWT claim 与 tenant/user 不一致时拒绝请求的业务校验。',
        code: 'curl -i -X POST "$RUNTIME_ENDPOINT/invoke" \\\n  -H "Authorization: Bearer $RUNTIME_API_KEY" \\\n  -H "user_id: user-001" -H "session_id: identity-check" \\\n  -d \'{"prompt":"你好"}\'',
        file: 'platform_request_context.py + platform_capabilities.py',
      },
    ],
  },
  session: {
