# ADP ClawAgent 导出物迁移至 AgentKit Runtime 示例

## 概述

本示例演示如何将 ADP / TCADP ClawAgent 导出的应用包迁移为可部署到 AgentKit Runtime 的 VeADK 工程。

ADP / TCADP ClawAgent 导出包不是可直接发布到 AgentKit Runtime 的 Python 工程。其应用信息、Agent 指令、模型配置、工具配置和运行配置主要保存在平台导出的 JSON 文件中。对于这类低代码平台生成的 Agent 项目，可结合 AgentKit Migration 命令与 AgentKit Codex Sandbox 的能力完成自动化迁移：使用 `agentkit migrate any_input --framework any create` 创建迁移任务并提交导出包，由远端 Codex Sandbox 分析应用配置、Agent 配置和可见行为，并生成可部署到 AgentKit Runtime 的 VeADK 工程。

本示例输入是一个旅行规划类 ClawAgent 应用。源应用是中国本地旅行规划助手，包含北京、成都、杭州三城旅游资料、预算判断规则和同行人规则。迁移后的输出是一个经过codex转化过的VeADK项目，并且您可以直接将此项目部署到Agentkit Runtime当中。

本文档将引导你完成导出包准备、远端迁移任务创建、结果查询下载、迁移产物检查，以及通过 `agentkit release` 发布到 AgentKit Runtime。

## 核心功能

本示例覆盖从 ADP / TCADP ClawAgent 导出包到 AgentKit Runtime 工程的完整迁移路径：

- 读取 ClawAgent 导出包中的应用元信息、Agent 指令、模型配置、工具配置和可见行为。
- 通过远端 Codex Sandbox 将 ClawAgent 应用转换为 VeADK / AgentKit Runtime 工程。
- 生成可部署配置、运行代码、迁移报告和评测样例，并支持继续执行 `agentkit release`。
- 对源应用中不存在的实时数据、外部 API 或真实库存查询能力，在迁移产物中保留能力边界说明，不伪造外部调用结果。

## 原输入

输入目录是 `any_input/`，其中包含从 ADP / TCADP 平台导出的 ClawAgent 应用包。准备方式如下：在平台应用列表中选择目标应用并导出，下载 ZIP 文件后解压，再将解压后的导出目录整体放入 `any_input/`。
...
```text
ADP / TCADP ClawAgent 导出 ZIP
    ↓
解压后放入 any_input/
    ↓
agentkit migrate any_input --framework any create
    ↓
远端 Codex Sandbox 分析导出包结构和 Agent 行为
    ↓
...
在 `ADP/claw/` 目录下复制 `.env.example` 为 `.env`，并在 `.env` 中填写迁移和部署所需的环境变量。
...
`create` 会把 `any_input/` 提交到远端 Codex Sandbox 执行迁移。`--output ../any_output` 以 `any_input/` 为基准解析，最终产物会写入 `ADP/claw/any_output/`。
...
创建成功后，命令会返回 `job_id`。本示例的实测任务记录保存在 `any_input/.agentkit/migrate/jobs/<job_id>.json`，迁移结果可下载到 `any_output/`。
...
- `assistant/agent.py`：迁移后的 VeADK agent，包含旅行规划助手人设、城市资料、预算判断规则、同行人规则和能力边界说明。
...
本示例输出中，源 ClawAgent 被转换为一个名为 `travel_planner` 的 VeADK agent。它保留中国本地旅行规划、预算判断、交通建议和结构化行程输出能力，并在用户要求实时票价、开放时间、排队时间或库存状态时说明当前能力边界。

## 示例提示词

- 我想带父母去北京玩 3 天，总预算 3000 元，喜欢历史文化、胡同和老北京美食，行程轻松一点。请帮我规划每天的景点、美食和交通建议。
- 我想去成都玩 2 天，预算 1800 元，喜欢当地小吃和轻松的城市路线，请安排一个不太累的路线。
...
- 迁移命令会改写 `any_input/` 中的源导出包吗？

  不会。迁移任务会把源导出包作为输入上传分析，并将生成的 AgentKit Runtime 工程写入 `--output` 指定目录。
