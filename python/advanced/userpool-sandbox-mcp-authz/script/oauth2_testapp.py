"""Small local OAuth2 app for getting a User Pool access token."""

from __future__ import annotations

import html
import os

import jwt as pyjwt
from authlib.integrations.flask_client import OAuth
from dotenv import load_dotenv
from flask import Flask, redirect

load_dotenv()

app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "dev-secret-key")

issuer = os.environ.get("OAUTH2_ISSUER_URI", "").rstrip("/")
client_id = os.environ.get("OAUTH2_CLIENT_ID")
client_secret = os.environ.get("OAUTH2_CLIENT_SECRET")
redirect_uri = os.environ.get(
    "OAUTH2_TESTAPP_REDIRECT_URI",
    "http://127.0.0.1:8082/callback",
)
scopes = os.environ.get("OAUTH2_SCOPES", "openid profile email")

oauth = OAuth(app)
if issuer and client_id:
    oauth.register(
        name="ciam",
        client_id=client_id,
        client_secret=client_secret,
        server_metadata_url=f"{issuer}/.well-known/openid-configuration",
        client_kwargs={"scope": scopes},
        redirect_uri=redirect_uri,
    )


@app.get("/")
def index():
    if not issuer or not client_id:
        return (
            "<html><body><h1>User Pool token helper</h1>"
            "<p>Set OAUTH2_ISSUER_URI and OAUTH2_CLIENT_ID first.</p>"
            "</body></html>"
        )
    return (
        "<html><body><h1>User Pool token helper</h1>"
        "<p><a href='/login'>Sign in</a></p></body></html>"
    )


@app.get("/login")
def login():
    return oauth.ciam.authorize_redirect(redirect_uri=redirect_uri, state="state")


@app.get("/callback")
def callback():
    token = oauth.ciam.authorize_access_token()
    access = token.get("access_token", "")
    claims = pyjwt.decode(access, options={"verify_signature": False})
    username = html.escape(str(claims.get("preferred_username", "")))
    email = html.escape(str(claims.get("email", "")))
    items = "".join(
        f"<li><strong>{html.escape(str(k))}:</strong> {html.escape(str(v))}</li>"
        for k, v in claims.items()
    )
    escaped_access = html.escape(access)
    return f"""
    <html>
      <body>
        <h1>User Info</h1>
        <p><strong>preferred_username:</strong> {username}</p>
        <p><strong>email:</strong> {email}</p>
        <h2>Access Token Claims</h2>
        <ul>{items}</ul>
        <p><strong>Access Token:</strong></p>
        <textarea rows="10" cols="120" readonly>{escaped_access}</textarea>
        <p><a href='/logout'>Logout</a></p>
      </body>
    </html>
    """


@app.get("/logout")
def logout():
    return redirect("/")


if __name__ == "__main__":
    app.run(host="127.0.0.1", port=8082)
