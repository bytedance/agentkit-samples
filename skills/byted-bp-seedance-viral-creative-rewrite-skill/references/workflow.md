...
3. Bundle BytePlus registration, API key setup, Seedance 2.5 resource package readiness, and rehearsal/real-generation choice into one opening message.
   ...
   - Say in the opening that real generation requires a ModelArk API Key and Seedance 2.5 resource package or entitlement, and that the user will first see the brief, strategy, forbidden carryover, and risk controls before the Seedance submission step. Do not include the final confirmation phrase in the opening.
...
16. Before real generation, collect or confirm `ARK_API_KEY` and Seedance 2.5 resource package or equivalent entitlement.
