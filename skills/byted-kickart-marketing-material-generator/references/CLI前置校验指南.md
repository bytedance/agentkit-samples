...
所有一键成片操作必须通过 `npm exec -y --package @volcengine/kickart-open-mcp@1.0.x kickart-open-cli` 执行，不得直接请求 OpenAPI。
...
| `npm exec -y --package @volcengine/kickart-open-mcp@1.0.x kickart-open-cli submit_marketing_task_v2` | 提交一键成片任务 | `task_id`、`request_id`、`request_body` |
| `npm exec -y --package @volcengine/kickart-open-mcp@1.0.x kickart-open-cli query_task_v2` | 查询一键成片任务 | `status`、`progress`、`result_url`、`response_file_path` |
...
7. 一键成片使用 `submit_marketing_task_v2`，模板默认 `978755842`，`--duration` 使用 `15`、`30`、`45` 或 `60`。
