...
description: 短视频平台爆款视频裂变。支持参考热门视频的结构，生成风格类似的视频。当用户提及"爆款裂变"，或表达希望参考某热门视频的风格、制作风格类似内容的等价意图时使用本技能。
version: 2.3.1
---
...
|---------|----------------|----------------------|---------|---------|
| **进度查询** | 【最高优先级】用户提及「查询进度」「查看结果」「生成怎么样了」「好了吗」或查询某个具体任务ID的状态时。 | 读取 `references/任务查询指南.md`，执行 `npm exec -y --package @volcengine/kickart-open-mcp@1.0.x kickart-open-cli query_task_v2 --task-id <ID> --source ad_variations --auto-publish true`。 | 解析 stdout JSON 并输出当前状态或结果。 | 需从会话上下文获取对应任务 ID；无有效 ID 时直接告知用户未查询到相关任务。 |
...
| **爆款裂变** | 用户提及「爆款裂变」、「爆款克隆」、「复制视频」、「克隆视频」、「同款视频」、「生成同款」、「裂变视频」等，或者表达希望制作和某热门视频一样的内容的等价意图时。 | 读取 `references/爆款裂变指南.md`，执行 `npm exec -y --package @volcengine/kickart-open-mcp@1.0.x kickart-open-cli submit_viral_task_v2` | 解析 stdout JSON 并输出任务 ID；后续通过 `query_task_v2` 获取结果 | 必须严格按照指南 Phase 0 逐步收集参考视频、商品信息和语言；角色图片、场景图片、创意提示词、字幕、字幕擦除、明水印等可选参数也必须逐一询问。完成信息收集后必须展示成片信息，并进行二次确认与合规提示，未确认前严禁提交。 |
| **视频参考** | 用户主动提供参考视频、视频链接，或表示要上传视频时 | 读取 `references/视频参考指南.md`，校验并记录本地绝对路径或公网 URL；不预先上传 | 获取可直接传给 `submit_viral_task_v2 --ref-video` 的素材值 | 用户触发爆款裂变意图时首要的前置步骤 |
| **数字形象** | 用户提及「添加模特」「自定义角色」「模特出镜」「数字人」「自定义模特」时 | 读取 `references/数字形象指南.md`，校验并记录本地绝对路径或公网 URL；不预先上传 | 获取可直接传给 `submit_viral_task_v2 --model-images` 的图片列表 | 用户在裂变任务中需要指定角色出镜时触发 |
...
| 1410 | TemplateIdNotExist | 模板不存在 | 请求的 template_id 无效或对应的模板未上线 | 请确认模板ID是否正确（当前模板ID: 978755330） |
